...
REMINDER_OWNER_ID = ids.SHARED_SPACE_OWNER_UUID
REMINDER_SPACE_ID = ids.SHARED_SPACE_ID
REMINDER_ID = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"

REMINDER_OWNER_SUBJECT = resource.format_ref(fields.ResourceType.USER, REMINDER_OWNER_ID)
REMINDER_RESOURCE = resource.format_ref(fields.ResourceType.REMINDER, REMINDER_ID)
REMINDER_SPACE_SUBJECT = resource.format_ref(fields.ResourceType.SPACE, REMINDER_SPACE_ID)

ONE_HOUR_SECONDS = 60 * 60

SPACE_CREATED_REQUEST_ID = str(uuid.uuid4())
REMINDER_CREATED_REQUEST_ID = str(uuid.uuid4())
...
class TestUpdateResource:
    ...
    @pytest.mark.parametrize(
        ("case", "expected_audit_message"), # noqa: WPS226
        [
            pytest.param(
                UpdateResourceCase(
    ...
    def test_create_note(
        ...
        if message and expected_audit_message is not None: # noqa: WPS204
        ...
        [
            pytest.param(
                UpdateResourceCase(
                    x_telegram_user_id=ids.ADMIN_USER_ID,
                    request=resource.ResourceChangeMessage(
                        request_id=REMINDER_CREATED_REQUEST_ID,
                        resource=resource.ResourceRef(
                            type=fields.ResourceType.REMINDER, id=REMINDER_ID
                        ),
                        operation=fields.Operation.CREATE,
                        change_type=fields.ChangeType.RESOURCE_ADDED,
                        relations=resource.ResourceRelations(
                            owner=resource.ResourceRef(
                                type=fields.ResourceType.USER, id=REMINDER_OWNER_ID
                            ),
                            parent=resource.ResourceRef(
                                type=fields.ResourceType.SPACE, id=REMINDER_SPACE_ID
                            ),
                        ),
                        context=resource.ResourceEventContext(
                            source_service=fields.NOTES_SERVICE_NAME,
                            event_type=fields.EventType.REMINDER_CREATED,
                        ),
                    ),
                    expected_status_code=httpx.codes.OK,
                    expected_response=resource.ResourceChangeResponse(
                        ...
                        written_tuples=(
                            resource.AuthTuple(
                                subject=REMINDER_OWNER_SUBJECT,
                                relation=fields.Relation.OWNER,
                                resource=REMINDER_RESOURCE,
                            ),
                            resource.AuthTuple(
                                subject=REMINDER_SPACE_SUBJECT,
                                relation=fields.Relation.SPACE,
                                resource=REMINDER_RESOURCE,
                            ),
                        ),
            ...
            pytest.param(
                ...
                audit.AuditMessage(
                    ...
                    cause=(
                        f'POST validation error for Write POST with body {{"code":"write_failed_due_to_invalid_input",'
                        f'"message":"cannot write a tuple which already exists: user: \'user:{REMINDER_OWNER_ID}\', relation: \'owner\', object: \'reminder:{REMINDER_ID}\': tuple to be written already existed or the tuple to be deleted did not exist"}}\n'
                        f" with error code write_failed_due_to_invalid_input error message: cannot write a tuple which already exists: user: 'user:{REMINDER_OWNER_ID}', relation: 'owner', object: 'reminder:{REMINDER_ID}': tuple to be written already existed or the tuple to be deleted did not exist"
                    ),
                    error_code=audit.ErrorCode.WRITE_FAILED_DUE_TO_INVALID_INPUT,
                    kind=audit.Kind.VALIDATION,
                    operation=audit.Operation.FGA_UPDATE_RESOURCE,
            ...
            pytest.param(
                ...
                audit.AuditMessage(
                    ...
                    context={
                        fields.USER_AGENT_FIELD: audit.TEST_USER_AGENT,
                        fields.USER_ID_FIELD: ids.INVALID_USER_ID,
                    },
                ),
                id="user not found",
            ),
    ...
    def test_create_reminder(     
        ...
        auth_service_error_messages_from_rabbitmq: ContextManager[Optional[bytes]],) -> None:
        """
        Тест на создание напоминания.
        - Успешное создание напоминания: 200 OK
        - Напоминание уже существует: 409 Conflict
        - Пользователя не существует (хедер x-telegram-user-id): 404 Not Found
        """
        token = login
